    ...
    const checkPendingBooking = async () => {
      try {
        ...
        const pending = myBookings.find(
          (b) => b.event_id === Number(id) && b.status === 'PENDING' && b.expires_at && new Date(b.expires_at).getTime() > Date.now()
        );
        if (pending) {
        ...
        <div className="bg-amber-50 border-b border-amber-200">
          <div className="container mx-auto px-4 py-3 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-amber-800">Bạn đang có {pendingBooking.items?.length || 0} ghế đang giữ chỗ!</p>
                <p className="text-xs text-amber-600">Hết hạn lúc {new Date(pendingBooking.expires_at).toLocaleTimeString('vi-VN')} — Quay lại thanh toán để không mất ghế.</p>
              </div>
            </div>
            <button
              onClick={() => navigate(`/checkout?bookingId=${pendingBooking.id}`, { state: { bookingId: pendingBooking.id } })}
              className="px-5 py-2 bg-amber-600 text-white text-sm font-bold rounded-full hover:bg-amber-700 transition-colors whitespace-nowrap shadow-sm"
            >
              Quay lại thanh toán
            </button>
          </div>
