...
import { Event, Seat, Zone, AdminBooking } from "../../types";
...
export function AdminEventsPage() {
  ...
  const [liveTab, setLiveTab] = useState<'seats' | 'customers'>('seats');
  const [eventBookings, setEventBookings] = useState<AdminBooking[]>([]);
  ...
  return (
    <div>
      ...
      {selectedLiveEventId && (
        ...
        >
          <div
            className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl h-[92vh] flex flex-col p-6"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-start justify-between gap-4 mb-6">
              <div>
                <h3 className="text-2xl font-bold text-slate-800 mb-2">
                  Chi tiết quản lý sự kiện
                </h3>
            ...
            <div className="flex gap-2 mb-6">
              <button
                onClick={() => setLiveTab('seats')}
                className={`px-5 py-2 rounded-full text-sm font-bold transition-all ${liveTab === 'seats' ? 'bg-slate-800 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
              >
                Trạng thái ghế
              </button>
              <button
                onClick={() => {
                  setLiveTab('customers');
                  if (selectedLiveEventId && eventBookings.length === 0) {
                    setLoadingEventBookings(true);
                    adminBookingService.getBookings().then(allBookings => {
                      const filtered = allBookings.filter(b => b.event_id === selectedLiveEventId);
                      setEventBookings(filtered);
                    }).catch(console.error).finally(() => setLoadingEventBookings(false));
                  }
                }}
                className={`px-5 py-2 rounded-full text-sm font-bold transition-all ${liveTab === 'customers' ? 'bg-slate-800 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
              >
                Khách hàng đặt vé
              </button>
            </div>
                    ...
                    <div>
                      {loadingEventBookings ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-16 text-center">
                          <Armchair className="w-10 h-10 text-slate-200 mb-3" />
                          <p className="text-sm font-semibold text-slate-400">Chưa có ai đặt vé</p>
                          <p className="text-xs text-slate-400 mt-1">Sự kiện này chưa có khách hàng nào đặt vé.</p>
                        </div>
                      ) : (
                          ...
                          <table className="w-full text-left">
                            <thead>
                              <tr className="border-b border-slate-200">
                                <th className="py-3 px-4 text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">#</th>
                                <th className="py-3 px-4 text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Họ và tên</th>
                                <th className="py-3 px-4 text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Số điện thoại</th>
                                <th className="py-3 px-4 text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Ghế</th>
                                <th className="py-3 px-4 text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Tổng tiền</th>
                                <th className="py-3 px-4 text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Thời gian đặt</th>
                                <th className="py-3 px-4 text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Trạng thái</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-50">
                              {eventBookings.map((booking, idx) => (
                                <tr key={booking.id} className="hover:bg-slate-50/50 transition-colors">
                                  ...
                                  <td className="py-3 px-4 text-sm font-bold text-slate-800">{booking.user_full_name || '---'}</td>
                                  <td className="py-3 px-4 text-xs font-medium text-slate-600">{booking.user_phone_number || '---'}</td>
                                  <td className="py-3 px-4">
                                    <div className="flex flex-wrap gap-1">
                                      {(booking.items || []).map(item => (
                                        <span key={item.id} className="px-2 py-0.5 rounded-md bg-cyan-50 text-cyan-700 text-[10px] font-bold border border-cyan-100">
                                          {item.seat_label}
                                        </span>
                                      ))}
                                    </div>
                                  </td>
                                  <td className="py-3 px-4 text-sm font-bold text-slate-800">{Number(booking.total_amount).toLocaleString()}đ</td>
                                  <td className="py-3 px-4 text-xs font-medium text-slate-600 whitespace-nowrap">
                                    {booking.updated_at ? new Date(booking.updated_at).toLocaleString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : '---'}
                                  </td>
                                  <td className="py-3 px-4">
                                    <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${booking.status === 'CONFIRMED' ? 'text-emerald-700 bg-emerald-50 border border-emerald-200' :
                                      booking.status === 'PENDING' ? 'text-amber-700 bg-amber-50 border border-amber-200' :
                                        'text-slate-600 bg-slate-50 border border-slate-200'
                                      }`}>
                                      {booking.status === 'CONFIRMED' ? 'Đã thanh toán' : booking.status === 'PENDING' ? 'Chờ thanh toán' : booking.status}
                                    </span>
                                  </td>
                                </tr>
                              ))}
